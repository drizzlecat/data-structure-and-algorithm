...
ics::pair<int,int> stats (const DTN& root) {
  int nodes = 1;
  int words = (root.is_word ? 1 : 0);
  for (const ics::pair<char, DTN>& j : root.children ) {
    ics::pair<int,int> nw = stats(j.second);
    nodes += nw.first;
    words += nw.second;
  }
  return ics::pair<int,int>(nodes,words);
  return ics::pair<int,int>(0,0);
  //Compiles but is incorrect
}
